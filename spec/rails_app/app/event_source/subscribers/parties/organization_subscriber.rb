...
module Parties
  class OrganizationSubscriber
    include ::EventSource::Subscriber[amqp: 'enroll.parties.organizations.fein_corrected']
    ...
    subscribe("on_faa.enroll.parties.organizations") do |delivery_info, metadata, payload|
      # Sequence of steps that are executed as single operation
      puts "triggered --> on_enroll_parties_organizations_fein_corrected block -- #{delivery_info} --  #{metadata} -- #{payload}"
    end
