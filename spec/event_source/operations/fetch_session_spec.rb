...
RSpec.describe EventSource::Operations::FetchSession do
  ...
  context "when session concern not defined" do
    before do
      allow(fetch_session).to receive(:session_concern_defined?).and_return(
        false
      )
    end
    ...
    it "should fail" do
      ...
      expect(result.failure).to eq "SessionConcern is not defined"
    end
  end
