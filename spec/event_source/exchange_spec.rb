...
RSpec.describe EventSource::Exchange do
  let(:server) do
    {
  ...
  context 'a queue_proxy is passed rather than an exchange_proxy' do
    context "a valid #{described_class}" do
      it 'bindings should match the AsyncApi configuration' do
        expect(
          described_class.new(exchange_proxy, publish_operation).bindings
        ).to eq publish_operation[:bindings]
      end
      ...
      context 'Adapter pattern methods are present' do
        ...
        it 'should have all the required methods' do
          expect(
            described_class.new(exchange_proxy, publish_operation)
          ).to respond_to(*adapter_methods)
        end
      end
    end
  end
