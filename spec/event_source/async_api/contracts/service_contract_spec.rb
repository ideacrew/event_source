...
RSpec.describe EventSource::AsyncApi::Contracts::ServiceContract do
  let(:asyncapi) { '2.0' }
  let(:id) { :adapter_id }
  ...
  let(:required_params) do
    { asyncapi: asyncapi, info: info, channels: channels }
  end
  ...
  context 'Given gapped or invalid parameters' do
    ...
    context 'and :asyncapi parameter only' do
      ...
      it do
        expect(
          subject.call({ asyncapi: asyncapi }).error?(:channels)
        ).to be_truthy
      end
      it do
        expect(subject.call({ asyncapi: asyncapi }).error?(:info)).to be_truthy
      end
    end
    ...
    context 'and :channels parameter only' do
      ...
      it do
        expect(
          subject.call({ channels: channels }).error?(:asyncapi)
        ).to be_truthy
      end
      it do
        expect(subject.call({ asyncapi: asyncapi }).error?(:info)).to be_truthy
