...
RSpec.describe EventSource::AsyncApi::Operations::Services::Create do
  let(:asyncapi) { '2.0' }
  ...
  describe '#call' do
    context 'with missing params' do
      let(:required_params) do
        { asyncapi: asyncapi, id: id, info: info, channels: channels }
      end
    ...
    context 'with required params' do
      ...
      it 'should successfully create an application instance' do
        ...
        expect(result.value!).to be_a EventSource::AsyncApi::Service

        expect(result.value!.to_h[:asyncapi]).to eq asyncapi
        expect(result.value!.to_h[:id]).to eq id

        expect(result.value!.info).to be_a EventSource::AsyncApi::Info
