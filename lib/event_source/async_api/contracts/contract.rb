      ...
      class Contract < Dry::Validation::Contract
        ...
        rule(:components).each do
          next unless key? && value do
                        ...
                        key.failure(text: "invalid component hash", error: result.errors.to_h) if result&.failure?
                      end
        end
        ...
        rule(:channels).each do
          next unless key? && value do
                        ...
                        # Use dry-validation metadata form to pass error hash along with text to calling service
                        key.failure(text: "invalid channel hash", error: result.errors.to_h) if result&.failure?
                      end
        end
        ...
        rule(:channel_items).each do
          next unless key? && value do
                        ...
                        # Use dry-validation metadata form to pass error hash along with text to calling service
                        key.failure(text: "invalid channel_item hash", error: result.errors.to_h) if result&.failure?
                      end
        end
        ...
        rule(:info) do
          if key? do
               ...
               # Use dry-validation metadata form to pass error hash along with text to calling service
               key.failure(text: "invalid info hash", error: result.errors.to_h) if result&.failure?
             end
          end
        end
        ...
        rule(:message) do
          if key? do
               ...
               # Use dry-validation metadata form to pass error hash along with text to calling service
               key.failure(text: "invalid message hash", error: result.errors.to_h) if result&.failure?
             end
          end
        end
        ...
        rule(:publish) do
          if key? do
               ...
               # Use dry-validation metadata form to pass error hash along with text to calling service
               key.failure(text: "invalid publish hash", error: result.errors.to_h) if result&.failure?
             end
          end
        end
        ...
        rule(:servers).each do
          next unless key? && value do
                        ...
                        # Use dry-validation metadata form to pass error hash along with text to calling service
                        key.failure(text: "invalid server hash", error: result.errors.to_h) if result&.failure?
                      end
        end
        ...
        rule(:tags).each do
          next unless key? && value do
                        ...
                        # Use dry-validation metadata form to pass error hash along with text to calling service
                        key.failure(text: "invalid tag hash", error: result.errors.to_h) if result&.failure?
                      end
        end
        ...
        rule(:external_docs).each do
          next unless key? && value do
                        ...
                        # Use dry-validation metadata form to pass error hash along with text to calling service
                        key.failure(text: "invalid external_doc hash", error: result.errors.to_h) if result&.failure?
                      end
        end
