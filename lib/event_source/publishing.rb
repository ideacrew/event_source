...
module EventSource
  ...
  module Publishing
    ...
    def publish_metadata(event_type, attributes = {})
      ...
      bus_attr['bus_app_hostname'] = ::EventSource.hostname
      ...
      ::EventSource.before_publish_callback(out)
      out
    end
    ...
    def publish(event_type, attributes = {})
      ...
      # ::EventSource.log_application("Event published: #{event_type} #{to_publish.inspect}")
      
      # enqueue_to(::EventSource.incoming_queue, ::EventSource::Driver, to_publish)
    ...
    def publish_at(timestamp_or_epoch, event_type, attributes = {})
      ...
      to_publish['bus_class_proxy'] = ::EventSource::Publisher.name.to_s

      ::EventSource.log_application("Event published:#{event_type} #{to_publish.inspect} publish_at: #{timestamp_or_epoch.to_i}")
      delayed_enqueue_to(timestamp_or_epoch.to_i, incoming_queue, ::EventSource::Worker, to_publish)
    end
    ...
    def enqueue_to(queue_name, class_name, hash)
      ...
      ::EventSource.adapter.enqueue(queue_name, ::EventSource::Worker, ::EventSource::Util.encode(hash || {}))
    end

    def delayed_enqueue_to(epoch_seconds, queue_name, klass, hash)
      ::EventSource.adapter.enqueue_at(epoch_seconds, queue_name, klass, ::EventSource::Util.encode(hash || {}))
    end
  end
end